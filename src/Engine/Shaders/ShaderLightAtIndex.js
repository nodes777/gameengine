...
/*global gEngine, vec3, eLightType */
...
        gl.uniform1i(this.mLightTypeRef, aLight.getLightType());
		...
		if(aLight.getLightType() === Light.eLightType.ePointLight){
			...
			// spot or directional lights: compute direction
			...
			if( aLight.getLightType() === Light.eLightType.eSpotLight){
				gl.uniform1f(this.mInnerRef, Math.cos(0.5 * aLight.getInner())); // cosine of half of inner angle, saves on computations
				gl.uniform1f(this.mOuterRef, Math.cos(0.5 * aLight.getOuter())); // cosine of half of outer cone
